...
template <class T> struct Matrix {
    ...
    Matrix(initializer_list<initializer_list<T>> init) : A(init.begin(), init.end()){};
    ...
    constexpr Matrix operator-(const Matrix &B) const { return (Matrix(*this) -= B); }
    constexpr Matrix operator*(const Matrix &B) const { return (Matrix(*this) *= B); }
    constexpr Matrix operator^(const long long k) const { return (Matrix(*this) ^= k); }
    constexpr bool operator==(const Matrix &B) const { return ((*this).A == B.A); }
